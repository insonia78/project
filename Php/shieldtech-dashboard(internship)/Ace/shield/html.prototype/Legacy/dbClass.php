...
**                                                **
** Adapted to MySQLi and additional modifications **
** by Ralph Mattiaccio							  **
***************************************************/
class Database
{
	...
	// public function send_sql_and_compensate($sql, $verbose, $match_array) {
	// 	if ( $verbose === true ) {
	// 		if ( $response = $this->send_sql($sql) ) {
	...
	// 					foreach ( $key in $match_array ) {
	// 						if (preg_match($match_array[$key], $response)) {
