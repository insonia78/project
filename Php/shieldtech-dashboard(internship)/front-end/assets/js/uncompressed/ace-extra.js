	...
	ace.settings = {
		...
		navbar_fixed : function(fix, save, chain) {
			...
			if (!fix && chain !== false) {
				...
				var sidebar = null;
				if (ace.settings.is('sidebar', 'fixed') || ((sidebar = document.getElementById('sidebar')) && ace.hasClass(sidebar, 'sidebar-fixed'))) {
			...
			if (window.jQuery) {
				jQuery(document).trigger('settings.ace', ['navbar_fixed', fix]);
			}
		},
		...
		sidebar_fixed : function(fix, save, chain) {
			...
			if (!fix && chain !== false) {
				...
				if (ace.settings.is('breadcrumbs', 'fixed') || ((breadcrumbs = document.getElementById('breadcrumbs')) && ace.hasClass(breadcrumbs, 'breadcrumbs-fixed'))) {
			...
			if (window.jQuery) {
				jQuery(document).trigger('settings.ace', ['sidebar_fixed', fix]);
			}
		},
		...
		breadcrumbs_fixed : function(fix, save, chain) {
			...
			if (window.jQuery) {
				jQuery(document).trigger('settings.ace', ['breadcrumbs_fixed', fix]);
			}
		},
		...
		main_container_fixed : function(inside, save) {
			...
			if (window.jQuery) {
				jQuery(document).trigger('settings.ace', ['main_container_fixed', inside]);
			}
		},
		...
		sidebar_collapsed : function(collpase, save) {
			...
			*/
			...
			if (window.jQuery) {
				jQuery(document).trigger('settings.ace', ['sidebar_collapsed', collpase]);
			}
		}
		/**
		,
