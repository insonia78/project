...
#include <QFile>
ProductionCostThread::ProductionCostThread(glpkLock *glpk,int identifierId, int row_id, DbConnector * database, QObject *parent) : QThread(parent) {
...
void ProductionCostThread::DeleteThread() {
    this->exit();
    //int count = 0;
    if (!this->wait(3000)) {
        this->terminate();
...
int ProductionCostThread::MonthForDay(int day) {
    if (day < 31) {
        return 1;
    } else if (day < 59) {
        return 2;
...
int ProductionCostThread::HoursForDay(int day) {
    return day * 24;
}

int ProductionCostThread::ProductionCost(int scenario_and_cases_row_id, int year) {
   
    qDebug() << "Production Cost Calculation started";
    double peakdemand1, energydemand1 = 0;// peakdemand2 = 0, energydemand2 = 0;
    ...
    try {
        ...
        while (res->next()) {
            ...
            if (!file.open(QIODevice::ReadOnly | QIODevice::Text)) {
                ...
                std::string query3("UPDATE scenarios_and_cases SET production_cost_calculation_status = 'ERROR in FILE'  WHERE row_id = ?;");
...
//    ProdCostIn.common_data.Ah = {hydroinvcost1};
    ...
    ProdCostIn.common_data.VCd = {demrescost_ds1};
    ...
    for (int i = 0; i < ND; i++) {
        value += QString::fromStdString("Demand Response Up " + std::to_string(i + 1)).append(",");
    }
    for (int i = 0; i < ND; i++) {
        value += QString::fromStdString("Demand Response Down " + std::to_string(i + 1)).append(",");
    }
    ...
    for (int i = 0; i < NHyd; i++) {
        value += QString::fromStdString("Hydro" + std::to_string(i + 1)).append(",");
    }
    for (int i = 0; i < NS; i++) {
        value += QString::fromStdString(stotype[i]+" Ch").append(",");
    }
    for (int i = 0; i < NS; i++) {
        value += QString::fromStdString(stotype[i]+" Dc").append(",");
    }
    /*for (int i = 0; i < NS; i++) {
    ...
    try {
        ...
        pstmt = con->prepareStatement("DELETE FROM production_cost_output_files WHERE ess_or_noess = 1 AND scenario_and_cases_row_id = " + std::to_string(scenario_and_cases_row_id));
        res = pstmt->executeQuery();
        //INSERT
        delete pstmt;
        delete res;
        ...
        pstmt->setBoolean(7, 1); //Noess is 0
        ...
        for (int generator = 0; generator < NGen + NR +NHyd; generator++) {
            ...
            pstmt = con->prepareStatement("DELETE FROM generator_calculated_metrics "
                    ...
                    + " and generator = ? and ess_or_noess = 1  ");
        ...
        pstmt = con->prepareStatement("DELETE FROM fuel_usage_output_installed_cap_tables WHERE scenario_and_cases_row_id = " + std::to_string(scenario_and_cases_row_id));
        res = pstmt->executeQuery();
        delete res;
        delete pstmt;
        separator = 0;
        separator2 = 0;
        qDebug() << " >> DELETE FROM fuel_usage_output_installed_cap_tables ";
        for (int generator = 0; generator < NGen; generator++) {
            ...
            //delete con;
            sql::PreparedStatement *pstmt;
            pstmt = con->prepareStatement("INSERT INTO fuel_usage_output_installed_cap_tables(outfuelburn1,outco2em1,thermtype,scenario_and_cases_row_id) VALUES (?,?,?,?)");
    ...
    {
        std::string query2("UPDATE scenarios_and_cases SET production_cost_calculation_status = 'NO SOLUTION', production_cost_calculation_date =CURRENT_TIMESTAMP() WHERE row_id = ?;");
...
int ProductionCostThread::ThreadSleep(int sleepTime) {
