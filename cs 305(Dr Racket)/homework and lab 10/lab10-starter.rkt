...
(define myflights 
  (list
   ...
   (make-flight "Boston" "New York" "daily")))
...
(check-expect(service-between? myflights "Boston" "New York")true)
...
;; >>>>>>Tests: USE the list myflights defined above to write enough check-expects to completely test your code.
;test
(check-expect(daily-to myflights "Boston")(list "Chicago" "New York"))
(check-expect(daily-to myflights "Chicago")(list "Boston"))
(check-expect(daily-to myflights "New York")(list "Boston"))
(check-expect(daily-to myflights "Paris")(list))

;; Definition: daily-to
 
 (define (daily-to alist city)
 (map
  (lambda (x)
   (cond
     [(string=?(flight-from x) city) (flight-to x)]
     [(string=?(flight-to x) city)(flight-from x)]
     [else empty]))
    (foldr cons empty
    (filter
      (lambda (x)
        (and
         (or
           (string=? (flight-from x) city)(string=? (flight-to x) city))
             (string=? (flight-frequency x) "daily")))alist))))
