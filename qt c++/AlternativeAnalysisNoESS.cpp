...
AlternativeAnalysisNoESSThread::AlternativeAnalysisNoESSThread(int row_id, DbConnector * database, QObject *parent) : QThread(parent) {
...
void AlternativeAnalysisNoESSThread::run() {
    qDebug() << this << " Start";
    AlternativeAnalysisNoESS(*row_id);        
    emit __terminateThread();    
    qDebug() << this << " Finished";

}
void AlternativeAnalysisNoESSThread::DeleteThread()
{
    this->exit();
...
AA_outputs AlternativeAnalysisNoESSThread::AlternativeAnalysisNoESS(int scenario_and_cases_row_id) {//,int NY) {
    ...
    //std:l:vector<double> heatrate,vom,p_reserve,s_reserve,t_reserve,carbon_rate,fixedom,sumderate,winderate,convramprate;
    ...
    try {
        ...
        //Read from calculation settings table
        ...
        while (res->next()) {
            ...
            if (!file2.open(QIODevice::ReadOnly | QIODevice::Text)) {
                ...
                delete pstmt;
                delete con;
                AA_outputs failed_demand2;
                return failed_demand2;;
            }
            else
                qDebug() << this << "File Opened";
        ...
        qDebug() << this << " >> programs_renewables_table";
    ...
    glp_set_prob_name(lp, "AlternativeAnalysisNoESS");
    ...
    counter += NGen;
    for (int i = 0; i < NEE; i++) {
        glp_set_col_bnds(lp, i + 1 + counter, GLP_DB, input_data.PexEE[i], input_data.PeeM[i]);
        glp_set_obj_coef(lp, i + 1 + counter, CostCoefficients[i + counter]);
        qDebug() << "DB Column:"<<i+counter<<", lb: "<<input_data.PexEE[i]<<" ub: " <<input_data.PeeM[i]<<" cc: "<<CostCoefficients[i+counter];
    }
    counter += NEE;
    for (int i = 0; i < ND; i++) {
    ...
    for (int i = 0; i < NS; i++) {
            glp_set_col_bnds(lp, i + 1 + counter, GLP_FX, input_data.PexS[i], 0.0);
            glp_set_obj_coef(lp, i + 1 + counter, CostCoefficients[i + counter]);
    ...
    {
        results.P_R.push_back(variables[i + extract_counter]);
    }
    extract_counter = extract_counter + NR;
    results.P_H.reserve(NHyd);
    for (int i = 0; i < NHyd; i++) //Hydro capacity
    {
        results.P_H.push_back(variables[i + extract_counter]);
    }
    ...
    {
        results.E_S.push_back(variables[i + extract_counter]);
        qDebug() << "Energy capacity: "<<results.E_S[i];
    }
    qDebug() << "Energy capacity size: " << results.E_S.size();
    extract_counter = extract_counter + NS;
...
void AlternativeAnalysisNoESSThread::TryLockingResponse(int value) {
    if (value) {
        tryLocking = &TRUE;
        qDebug() << ">> Db locked ";
    } else {
...
int AlternativeAnalysisNoESSThread::ThreadSleep(int sleepTime) {
