...
AlternativeAnalysisMultiYearNoESS::AlternativeAnalysisMultiYearNoESS(glpkLock* glpk, int identifierId, int row_id, DbConnector * database, QObject *parent) : QThread(parent) {
...
AlternativeAnalysisMultiYearNoESS::~AlternativeAnalysisMultiYearNoESS() {
    qDebug() << this << " >> Deleting";
}

void AlternativeAnalysisMultiYearNoESS::run() {
    qDebug() << this << " Start";
    AlternativeAnalysisMultiYearNoESSThread(*row_id, 2);
...
void AlternativeAnalysisMultiYearNoESS::DeleteThread() {
    this->exit();
...
int AlternativeAnalysisMultiYearNoESS::AlternativeAnalysisMultiYearNoESSThread(int scenario_and_cases_row_id, int NY) {//,int NY) {
    ...
    double temp_objective_value; // KAI
        ...
        while (res->next()) {
            for (int y = 1; y < NY + 1; y++) {
                ...
                if (!file.open(QIODevice::ReadOnly | QIODevice::Text)) {
                    ...
                    std::string query3("UPDATE scenarios_and_cases SET alternative_analysis_calculation_noess_status = 'ERROR in FILE'  WHERE row_id = ?;");
    ...
    for (int y = 0; y < NY; y++) {
        for (int i = 0; i < NHyd; i++) {
            for (int t = 0; t < NH; t++) {
                VCHF[y].push_back(hydrovomcost); // * (HoursperMonth[0] / 2 * (t < 2) + HoursperMonth[1] / 2 * (t < 4)*(t >= 2) + HoursperMonth[2] / 2 * (t < 6)*(t >= 4) + HoursperMonth[3] / 2 * (t < 8)*(t >= 6) + HoursperMonth[4] / 2 * (t < 10)*(t >= 8) + HoursperMonth[5] / 2 * (t < 12)*(t >= 10) + HoursperMonth[6] / 2 * (t < 14)*(t >= 12) + HoursperMonth[7] / 2 * (t < 16)*(t >= 14) + HoursperMonth[8] / 2 * (t < 18)*(t >= 16) + HoursperMonth[9] / 2 * (t < 20)*(t >= 18) + HoursperMonth[10] / 2 * (t < 22)*(t >= 20) + HoursperMonth[11] / 2 * (t >= 22))) / std::pow(1 + input_data.DiscRate, y));
            }
        }
    }
    ...
    for (int y = 0; y < NY; y++) {
        ...
        for (int i = 0; i < NS; i++) {
            glp_set_col_bnds(PROBLEM_VECTOR[y], i + 1 + counter, GLP_FX, input_data.PexS[y][i], 0.0);
            glp_set_obj_coef(PROBLEM_VECTOR[y], i + 1 + counter, CostCoefficients[i + counter]);
        ...
        // KAI : solution for linear equations, which could be NaN
        temp_objective_value = glp_get_obj_val(PROBLEM_VECTOR[y]);
    ...
    try {
        ...
        // KAI 4.2.2018
        pstmt = con->prepareStatement("DELETE FROM calculation_results_table WHERE calculation_id = 'Alternative Analysis No ESS' AND scenario_and_cases_row_id = " + std::to_string(scenario_and_cases_row_id));
        res = pstmt->executeQuery();


        // if objective_value equals to NaN then we are not inserting data
        if (results.objective_value != 0) {
            ...
            pstmt->setString(3, "Alternative Analysis No ESS");
            pstmt->executeUpdate();
        }
        delete res;
        delete pstmt;
        ...
        pstmt = con->prepareStatement("DELETE FROM installed_capacity_output_noess_cap_tables WHERE scenario_and_cases_row_id = " + std::to_string(scenario_and_cases_row_id));
        res = pstmt->executeQuery();
        /*
         for (int y = 0; y < NY; y++) {
        ...
        for (unsigned int i = 0; i < results.P_S[y].size(); i++) {
            //INSERT
            delete pstmt;
            pstmt = con->prepareStatement("INSERT INTO installed_capacity_output_noess_cap_tables(outinst1,outinst2,storage_type,scenario_and_cases_row_id,year_id) VALUES (?,?,?,?,?)");
            pstmt->setDouble(1, results.P_S[y][i]);
        ...
        pstmt = con->prepareStatement("DELETE FROM demand_side_output_installed_cap_noess_tables WHERE scenario_and_cases_row_id = " + std::to_string(scenario_and_cases_row_id));
        res = pstmt->executeQuery();
        delete pstmt;

        //for (int y = 0; y < NY; y++) {
        //INSERT
        pstmt = con->prepareStatement("INSERT INTO demand_side_output_installed_cap_noess_tables(outeff1,outdem1,outeff2,outdem2,scenario_and_cases_row_id,year_id) VALUES (?,?,?,?,?,?)");
        ...
        pstmt = con->prepareStatement("DELETE FROM energy_capacity_output_noess_cap_tables WHERE scenario_and_cases_row_id = " + std::to_string(scenario_and_cases_row_id));
        res = pstmt->executeQuery();
        //for (int y = 0; y < NY; y++) {
        for (int i = 0; i < NS; i++) {
            //INSERT
            delete pstmt;
            pstmt = con->prepareStatement("INSERT INTO energy_capacity_output_noess_cap_tables(outener1,outener2,storage_type,scenario_and_cases_row_id,year_id) VALUES (?,?,?,?,?)");
        ...
        pstmt = con->prepareStatement("DELETE FROM hydro_generation_output_installed_cap_noess_tables WHERE scenario_and_cases_row_id = " + std::to_string(scenario_and_cases_row_id));
        res = pstmt->executeQuery();
        //INSERT
        delete pstmt;
        // KAI: PH [y][]: y=0 means year 1 
        //for (int y = 0; y < NY; y++) {
        pstmt = con->prepareStatement("INSERT INTO hydro_generation_output_installed_cap_noess_tables(outdam1,outdam2, scenario_and_cases_row_id,year_id) VALUES (?,?,?,?)");
        ...
        pstmt = con->prepareStatement("DELETE FROM thermal_generation_output_installed_cap_noess_tables WHERE scenario_and_cases_row_id = " + std::to_string(scenario_and_cases_row_id));
        res = pstmt->executeQuery();
        // for (int y = 0; y < NY; y++) {
        for (int i = 0; i < NGen; i++) {
            //INSERT
            ...
            pstmt = con->prepareStatement
                    ("INSERT INTO thermal_generation_output_installed_cap_noess_tables(outthermgen1 ,outthermgen2,fuel_type,scenario_and_cases_row_id,year_id) VALUES (?,?,?,?,?)");
        ...
        pstmt = con->prepareStatement("DELETE FROM renewables_output_installed_cap_noess_table WHERE scenario_and_cases_row_id = " + std::to_string(scenario_and_cases_row_id));
        res = pstmt->executeQuery();
        delete pstmt;
        ...
        for (int y = 0; y < NY; y++) {
            ...
            pstmt = con->prepareStatement
                    ("INSERT INTO renewables_output_installed_cap_noess_table(outwind1,outsolarpv1,outwind2,outsolarpv2,scenario_and_cases_row_id,year_id) VALUES (?,?,?,?,?,?)");
        ...
        std::cout << " (MySQL error code: " << e.getErrorCode();
        std::cout << ", SQLState: " << e.getSQLState() << " )" << std::endl;
        std::string query2("UPDATE scenarios_and_cases SET alternative_analysis_calculation_noess_status = 'ERROR_QUERY', alternative_analysis_calculation_noess_date =CURRENT_TIMESTAMP()  WHERE row_id = ?;");
        pstmt = con->prepareStatement(query2);
        pstmt->setInt(1, scenario_and_cases_row_id);
        pstmt->execute();
        delete pstmt;
    ...
    std::string query2("UPDATE scenarios_and_cases SET alternative_analysis_calculation_noess_status = 'COMPLETE', alternative_analysis_calculation_noess_date =CURRENT_TIMESTAMP()  WHERE row_id = ?;");
    pstmt = con->prepareStatement(query2);
    pstmt->setInt(1, scenario_and_cases_row_id);
    pstmt->execute();
    delete pstmt;
...
int AlternativeAnalysisMultiYearNoESS::ThreadSleep(int sleepTime) {
