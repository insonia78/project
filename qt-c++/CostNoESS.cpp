...
ProductionCostNoESSThread::ProductionCostNoESSThread(int row_id, DbConnector * database, QObject *parent) : QThread(parent) {
    this->database = database;
    temp_row = row_id;
    this->row_id = &temp_row;
   // QObject::connect(this, SIGNAL(_tryLocking()), database, SLOT(__tryLocking()), Qt::QueuedConnection);
   // QObject::connect(database, SIGNAL(_tryLockingResponse(int)), this, SLOT(TryLockingResponse(int)), Qt::UniqueConnection);
...
int ProductionCostNoESSThread::HoursForDay(int day){
    return day*24;
}

int ProductionCostNoESSThread::MonthForDay(int day){
    if(day < 31){
        return 1;
    }
    else if (day < 59){
...
void ProductionCostNoESSThread::run() {
    AA_outputs AAresults;
    ProductionCostNoESS(*row_id, 1,AAresults);
    emit _terminateThread();
    qDebug() << this << " Finished";

}

void ProductionCostNoESSThread::DeleteThread() {
    this->exit();
...
PC_outputs ProductionCostNoESSThread::ProductionCostNoESS(int scenario_and_cases_row_id, int year, AA_outputs AAresults) {
    qDebug() << "Production Cost NoESS Calculation started";
    ...
    try {
        ...
        while (res->next()) {
            ...
            if (!file.open(QIODevice::ReadOnly | QIODevice::Text)) {
                ...
                std::string query3("UPDATE scenarios_and_cases SET storage_benefits_calculation_status = 'ERROR in FILE'  WHERE row_id = ?;");
                pstmt = con->prepareStatement(query3);
                pstmt->setInt(1, scenario_and_cases_row_id);
                pstmt->execute();
                delete pstmt;
                delete con;
                PC_outputs failed_demand;
                return failed_demand;
            } else
            ...
            if (!file2.open(QIODevice::ReadOnly | QIODevice::Text)) {
                ...
                std::string query2("UPDATE scenarios_and_cases SET storage_benefits_calculation_status = 'ERROR in FILE' WHERE row_id = ?;");
        ...
        qDebug() << this << " >> generation_renewables_table";
        ...
        delete pstmt;
        //Read Hydro table
        qDebug() << this << " >> generation_hydro_table";
        ...
        qDebug() << this << " >> hydro_monthly_energy_table";
        ...
        qDebug() << this << " >> programs_planning_criteria_table";
        ...
        qDebug() << this << " >> programs_demand_side_table";
        ...
        qDebug() << this << " >> energy_storage_dynamic_table";
        ...
        qDebug() << this << " >> energy_storage_cost_dynamic_table";
        ...
        qDebug() << this << " >> fuel_price_forecast_table";
        ...
        qDebug() << this << " >> generation_conventional_table";
        ...
        qDebug() << this << " >> tech_capital_dynamic_table";
        ...
        PC_outputs failed_database;
        return failed_database;
    }    
    std::vector<double> not_normal, not_normal_status;
    //Read installed ESS power capacity
    for (unsigned int s=0;s<AAresults.P_S.size();s++){
        SolPS.push_back(AAresults.P_S[s]);
    }
    for (unsigned int i = 0; i < SolPS.size(); i++) {
        qDebug() << "ES power investment: " << SolPS[i];
    }
    //Read demand side installed capacity
    for (unsigned int d=0;d<AAresults.P_DR.size();d++){
        SolPEE.push_back(AAresults.P_ee[d]);
        SolPD.push_back(AAresults.P_DR[d]);
    }
    ...
    for (unsigned int s=0;s<AAresults.P_H.size();s++){
        SolPH.push_back(AAresults.P_H[s]);
    }
    //        for (int i = 0; i < SolPH.size(); i++){
    //            qDebug() << "Hyd investment: " << SolPH[i];
    //        }
    //Read Thermal installed capacity
    for (unsigned int s=0;s<AAresults.P_peak.size();s++){
        SolPPeak.push_back(AAresults.P_peak[s]);
    }
    //        for (int i = 0; i < SolPPeak.size(); i++){
    //            qDebug() << "Thermal investment: " << SolPPeak[i];
    //        }
    //Read installed renewable capacity
    for (unsigned int s=0;s<AAresults.P_R.size();s++){
        SolPR.push_back(AAresults.P_R[s]);
    }
    ...
    for (unsigned int i = 0; i < ProdCostIn.common_data.Fee.size(); i++) {
    ...
    for (int day = 0; day < NUMBER_OF_DAYS; day++) {
        glp_set_prob_name(PROBLEM_VECTOR[day], "ProductionCostNoESS");
        ...
        qDebug() << "Discharge size: " <<results.G_DS.size();
        extract_counter = extract_counter + NS * NH;
...
//    std::string GenFileName = "PC_generation_";
...
//    }
//
//    std::string PRFileName = "PC_primres_";
...
//    }
//
//    std::string SRFileName = "PC_secres_";
...
//        }
//    }
//    std::string TRFileName = "PC_tertres_";
...
//        }
//    }
//    std::string EPFileName = "PC_enerprice_";
...
//    }
//
//    std::string RPFileName = "PC_resprice_";
...
//            out10 << QString::number(results.RP_price[day * NH + t]).toUtf8() << "," << QString::number(results.RS_price[t]).toUtf8() << "," << QString::number(results.RT_price[t]).toUtf8() << QString::fromStdString("\n").toUtf8(); //"," << results.RT_price[t] << QString::fromStdString("\n");
//        }
//    }
    return results;
...
void ProductionCostNoESSThread::TryLockingResponse(int value) {
    if (value) {
        tryLocking = &TRUE;
        qDebug() << ">> Db locked ";
    } else {
...
int ProductionCostNoESSThread::ThreadSleep(int sleepTime) {
