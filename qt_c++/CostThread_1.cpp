...
ProductionCostThread_1::ProductionCostThread_1(glpkLock *glpk,int identifierId, int row_id, DbConnector * database, QObject *parent) : QThread(parent) {
...
ProductionCostThread_1::~ProductionCostThread_1() {
    qDebug() << this << " >> Deleting";
}

void ProductionCostThread_1::run() {
...
void ProductionCostThread_1::DeleteThread() {
    this->exit();
    int count = 0;
    if (!this->wait(3000)) {
        this->terminate();
...
int ProductionCostThread_1::MonthForDay(int day) {
    if (day < 31) {
        return 1;
    } else if (day < 59) {
        return 2;
...
int ProductionCostThread_1::HoursForDay(int day) {
    return day * 24;
}

int ProductionCostThread_1::ProductionCost(int scenario_and_cases_row_id, int year) {
    ...
    ProdCostIn.common_data.Fee = {enereff7};
    ...
    ProdCostIn.common_data.PeeM = {enereff11};
    ...
    for (int i = 0; i < ProdCostIn.common_data.Fee.size(); i++) {
    ...
    std::string query2("UPDATE scenarios_and_cases SET emulator_calculation_status = 'COMPLETE', production_cost_calculation_date =CURRENT_TIMESTAMP() WHERE row_id = ?;");
...
void ProductionCostThread_1::TryLockingResponse(int value, QString call) {
...
void ProductionCostThread_1::TryLockingResponseGlpk(int value, QString call) {
    if (QString::compare(call, identifier) == 0) {
        if (value) {
            tryGlpkLocking = &TRUE;
            qDebug() << ">> Db locked ";
...
int ProductionCostThread_1::resetLocking() {
    qDebug() << this << " >> Reset locking";
    tryLocking = &FALSE;

}
int ProductionCostThread_1::resetGlpkLocking()
{
    qDebug() << this << " >> Reset locking";
